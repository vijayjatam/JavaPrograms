...
public class JugSolve_AI {
	...
	public static void puzzleStatus() {
		...
		for(int i=2;i<=30;i++) {
			for(int j=i+1;j<=30;j++) {
				for(int k=1;k<j;k++) {
					if(k!=i) {
						smallJugSize = JugSolve.smallJugSize = i;
						largeJugSize = JugSolve.largeJugSize = j;
						setSize = JugSolve.setSize = k;
						validate();
					}
				}
			}
	...
	public static void validate() {
		...
		JugSolve.smallJugSize = smallJugSize;
		JugSolve.largeJugSize = largeJugSize;
		if((setSize % JugSolve.GCD(smallJugSize,largeJugSize) != 0)) {
			...
			if (smallJugSize != largeJugSize) {
				...
				COUNT_ON_START_WITH_SMALL= JugSolve.fillSmallFirst();
				COUNT_ON_START_WITH_LARGE = JugSolve.fillLargeFirst();
